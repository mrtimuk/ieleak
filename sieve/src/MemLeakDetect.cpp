...
int MyTrace(wchar_t* lpszFormat, ...)
{
	wchar_t buffer[1024];
	char mbuffer[1024];
 	va_list args;
	...
	vswprintf( buffer, lpszFormat, args );

	wcstombs(mbuffer,buffer,1024);

	return _CrtDbgReport(_CRT_WARN,NULL,NULL,NULL,mbuffer);
						 ...
						 int	lineNumber)
{
	...
	{
		TCHAR fbuf[MLD_MAX_NAME_LENGTH];
		if ( filename )
		{
			mbstowcs(fbuf,(const char *)filename,MLD_MAX_NAME_LENGTH);
		}
		else
		{
			_tcscpy(fbuf,_T(""));
		}
		g_pMemTrace->addMemoryTrace((void *) requestNumber, size, fbuf, lineNumber);
	}
	else
	if (allocType == _HOOK_REALLOC)
	{
		if (_CrtIsValidHeapPointer(userData))
		{
			...
			mbstowcs(fbuf,(const char *)filename,MLD_MAX_NAME_LENGTH);
	   		...
	   		g_pMemTrace->redoMemoryTrace((void *) requestNumber, (void *) prevRequestNumber, size, fbuf, lineNumber);
		}
	}
	else
...
void CMemLeakDetect::addMemoryTrace(void* addr,  DWORD asize,  TCHAR *fname, DWORD lnum)
{
	...
	{
		// already allocated
		AfxTrace(L"ERROR!CMemLeakDetect::addMemoryTrace() Address(0x%08X) already allocated\n", addr);
		return;
	}
	//
	ainfo.address		= addr;
...
void CMemLeakDetect::redoMemoryTrace(void* addr,  void* oldaddr, DWORD asize,  TCHAR *fname, DWORD lnum)
{
	AllocBlockInfo ainfo;
...
void CMemLeakDetect::removeMemoryTrace(void* addr, void* realdataptr)
{
	...
	else
	{
	   //freeing unallocated memory
		AfxTrace(L"ERROR!CMemLeakDetect::removeMemoryTrace() didnt find Address(0x%08X) to free\n", addr);
	}
};
...
void CMemLeakDetect::dumpMemoryTrace()
{
	...
	while(pos != m_AllocatedMemoryList.end())
	{
		...
		{
			AfxTrace(L"Memory Leak <0x%08X> bytes(%d) occurance(%d) %s(%d)\n", 
					ainfo.address, ainfo.size, ainfo.occurance, ainfo.fileName, ainfo.lineNumber);
		}
		else
		{
			AfxTrace(L"Memory Leak <0x%08X> bytes(%d) occurance(%d)\n", 
	...
	if(!totalSize) 
	{
		AfxTrace(L"No Memory Leaks Detected for %d Allocations\n\n", memoccurance);
	}
	else
	{
		AfxTrace(L"Total %d Memory Leaks: %d bytes Total Alocations %d\n\n", numLeaks, totalSize, memoccurance);
	}
}
...
BOOL CMemLeakDetect::initSymInfo( TCHAR* lpszUserSymbolPath )
{
	...
	char	lpszSymbolPathMb[MLD_MAX_NAME_LENGTH];
	...
	wcstombs( lpszSymbolPathMb, lpszSymbolPath, MLD_MAX_NAME_LENGTH );
	return SymInitialize( GetCurrentProcess(), lpszSymbolPathMb, TRUE);
}
...
void CMemLeakDetect::symStackTrace(STACKFRAMEENTRY* pStacktrace )
{
	...
	{
       AfxTrace(L"Call stack info(thread=0x%08X) failed.\n", hThread );
	   return;
	}
...
BOOL CMemLeakDetect::symFunctionInfoFromAddresses( ULONG fnAddress, ULONG stackAddress, TCHAR* lpszSymbol )
{
	DWORD             dwDisp	= 0;
	...
	{
		mbstowcs(lpszSymbol, m_pSymbol->Name, MLD_MAX_NAME_LENGTH);
		return TRUE;
	}
	//create the symbol using the address because we have no symbol
	_stprintf(lpszSymbol, L"0x%08X", fnAddress);
	return FALSE;
}
...
BOOL CMemLeakDetect::symSourceInfoFromAddress(UINT address, TCHAR* lpszSourceInfo)
{
	...
	if ( SymGetLineFromAddr( m_hProcess, address, &dwDisp, &lineInfo ) )
	{
	   // Using the "sourcefile(linenumber)" format
		mbstowcs(fbuf,lineInfo.FileName,MLD_MAX_NAME_LENGTH);
		_stprintf( lpszSourceInfo, _T("%s(%d): 0x%08X"), fbuf, lineInfo.LineNumber, address );
		ret = TRUE;
	}
	else
	{
...
BOOL CMemLeakDetect::symModuleNameFromAddress( UINT address, TCHAR* lpszModule )
{
	...
	TCHAR			  mbuf[MLD_MAX_NAME_LENGTH];
	...
	{
		mbstowcs(lpszModule,moduleInfo.ModuleName,MLD_MAX_NAME_LENGTH);
		ret = TRUE;
	}
	else
	{
